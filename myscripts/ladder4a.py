"""
Ladder step 4a: learned 1st-order DS from one self-intersecting capricorn demonstration.

Trains PUMA on LAIR capricorn episode 0, then simulates the learned field from a grid of
initial positions (compare with ladder3a curved non-self-intersecting demo).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ladder 4a: learned 1st-order DS on one capricorn demo (episode 0).",
    )
    parser.add_argument(
        "--train-only",
        action="store_true",
    ...
    parser.add_argument(
        "--save",
        type=str,
        default=os.path.join(os.path.dirname(__file__), "images", "ladder4a_learned_ds.png"),
