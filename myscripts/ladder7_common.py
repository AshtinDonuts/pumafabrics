"""
Shared utilities for ladder step 7: DS transported under non-uniform scaling.

A source field ``f(x)`` defined in source coordinates is pushed to target coordinates
``y = S x + t`` (diagonal scale matrix ``S = diag(s_x, s_y)``, optional translation ``t``)
via

    dy/dt = S f(S^{-1} (y - t))          (1st order)
...
class NonUniformScaleTransform2D:
    """Affine map y = S @ x + t with diagonal non-uniform scale S = diag(s_x, s_y)."""
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        return (self.matrix @ x.T).T + self.translation

    def inverse(self, y: np.ndarray) -> np.ndarray:
        y = np.asarray(y, dtype=float)
        return (self.inv_matrix @ (y - self.translation).T).T
    ...
    def inverse_vector(self, v: np.ndarray) -> np.ndarray:
        v = np.asarray(v, dtype=float)
        return (self.inv_matrix @ v.T).T


def transform_states_2nd(states: np.ndarray, transform: NonUniformScaleTransform2D) -> np.ndarray:
...
def default_initial_states_target(
    order: OrderTag,
    transform: NonUniformScaleTransform2D,
    *,
...
class TransportedFirstOrderDS:
    """1st-order DS transported from a source field under non-uniform scaling."""
...
class TransportedSecondOrderDS:
    """2nd-order DS transported from a source field under non-uniform scaling."""
...
def plot_title(order: OrderTag, source: SourceTag, scale: np.ndarray) -> str:
    order_label = "1st-order" if order == "1st" else "2nd-order"
    return (
        f"Transported {order_label} DS ({SOURCE_LABELS[source]})\n"
        f"non-uniform scale ({scale[0]:.3g}, {scale[1]:.3g})"
    )
...
def run_transport_pipeline(
    ...
    *,
    ...
    transform = NonUniformScaleTransform2D.from_scale_and_translation(scale, translation)
    ...
    title = plot_title(order, source, transform.scale)
...
def add_transport_cli_args(parser) -> None:
    """Register ladder-7 CLI arguments on an ``ArgumentParser``."""
    parser.add_argument(
        "--source",
    ...
    parser.add_argument(
        ...
        default=[2.0, 0.5],
        metavar=("SX", "SY"),
        help="Non-uniform scale factors (s_x, s_y) in y = diag(s) x + t (default: 2.0 0.5).",
    )
    parser.add_argument(
        "--translation",
        type=float,
