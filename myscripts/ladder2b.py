"""
Ladder step 2b: learned 2nd-order DS from one straight-line demonstration.

Trains PUMA on a synthetic line segment, then simulates the learned field from a
grid of initial states [q, v] (compare with ladder1b analytic baseline).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ladder 2b: learned 2nd-order DS on one straight-line demo.",
    )
    parser.add_argument(
        "--train-only",
        action="store_true",
    ...
    parser.add_argument(
        "--save",
        type=str,
        default=os.path.join(os.path.dirname(__file__), "images", "ladder2b_learned_ds.png"),
