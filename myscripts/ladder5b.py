"""
Ladder step 5b: 2nd-order DS transported under translation and rotation.

Pulls a source field from any prior ladder step (analytic straight-line or learned
checkpoints from steps 2–4), applies a rigid transform, and simulates in the target frame.
"""
...
from ladder5_common import add_transport_cli_args, run_transport_pipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ladder 5b: transported 2nd-order DS (translation + rotation).",
    )
    add_transport_cli_args(parser)
    parser.add_argument(
        "--save",
        type=str,
        default=os.path.join(os.path.dirname(__file__), "images", "ladder5b_transported_ds_2nd_order.png"),
