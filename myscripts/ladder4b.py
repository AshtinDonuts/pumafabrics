"""
Ladder step 4b: learned 2nd-order DS from one self-intersecting capricorn demonstration.

Trains PUMA on LAIR capricorn episode 0, then simulates the learned field from a grid of
initial states [q, v] (compare with ladder3b curved non-self-intersecting demo).
"""
...
from ladder4_common import run_learned_ds_pipeline


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ladder 4b: learned 2nd-order DS on one capricorn demo (episode 0).",
    )
    parser.add_argument(
        "--train-only",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Ladder grid PNG every N iters while training (default: evaluation_interval, 1500). "
        "Use 0 to disable.",
    )
    parser.add_argument(
        "--no-plot",
    ...
    parser.add_argument(
        "--save",
        type=str,
        default=os.path.join(os.path.dirname(__file__), "images", "ladder4b_learned_ds.png"),
