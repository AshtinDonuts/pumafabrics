...
class Params:
    ...
    results_path: str = f'results/2nd_order_2D/{dataset_name}/'
    multi_motion: bool = False  # true when learning multiple motions together
    selected_primitives_ids: str = '8'  # zero-idx id number from dataset_keys.py, e.g., '2' or '4,0,6'
    ...
    learning_rate: float = 0.0001245  # 0.0001  # 0.0001  # AdamW learning rate
    ...
    stabilization_loss_weight: float = 0.447267  # 1  # 1  # stability loss weight
    boundary_loss_weight: float = 0  # boundary loss weight
    imitation_window_size: int = 15  # 15  # imitation window size
    stabilization_window_size: int = 12  # 5  # 2  # stability window size
    triplet_margin: float = 2.9194e-7  # 1.25e-4  # triplet loss margin
    ...
    max_iterations: int = 41000  #default: 41000 # maximum number of training iterations
