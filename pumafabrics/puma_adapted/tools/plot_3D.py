...
def _make_figure_from_pickle(obj, debug: bool) -> "go.Figure":
    ...
    # Try common trajectory keys.
    xyz = None
    for key in ("x_pos", "pos_fk"):
        if key in obj:
            xyz = obj[key]
            if debug:
                print(f"[plot_3D] detected format: trajectory dict (key: '{key}')", file=sys.stderr)
            break
    ...
    # Normalize to Nx3 list of floats.
    xs, ys, zs = [], [], []
    for p in xyz:
        if p is None:
            continue
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Path to a .pickle/.pkl/.pk file. If omitted, uses the legacy iteration-based default.",
    )
    parser.add_argument(
        "--iteration",
        default=None,
