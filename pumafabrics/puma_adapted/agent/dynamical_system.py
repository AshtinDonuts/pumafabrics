from pumafabrics.puma_adapted.agent.utils.dynamical_system_operations import batch_dot_product, denormalize_derivative, euler_integration, \
    normalize_state, denormalize_state, get_derivative_normalized_state
import torch
import numpy as np
...
class DynamicalSystem():
    ...
    def __init__(self, x_init, space, order, min_state_derivative, max_state_derivative, saturate_transition,
                 primitive_type,
        ...
        if self.space == 'sphere':
            ...
            x_init = torch.cat([x_init[:, :3], projected_points, x_init[:, self.dim_position:]],
                               dim=1)  # pytorch doesn't like inplace operations
    ...
    def integrate_2nd_order(self, x_t, acc_t_d):
        ...
        if self.space == 'euclidean':
            ...
            # Integrate non-euclidean
            pos_t_d_rot, vel_t_d_rot, acc_t_d_rot = self.integrate_non_euclidean_2nd_order(pos_t[:, 3:], vel_t[:, 3:],
                                                                                           acc_t_d[:, 3:])
    ...
    def obstacle_avoidance(self, x_t, dx_t, obstacles):
        ...
        if not obstacles['centers']:
            ...
            x_t = x_t.repeat_interleave(repeats=n_obs, dim=1).view(batch_size, self.dim_position, n_obs).transpose(1,
                                                                                                                   2)  # Repeat as many obstacles
            ...
            filter_i_eq_k = 1e30 * torch.eye(n_obs).repeat(batch_size, 1, 1).cuda() + torch.ones(n_obs).repeat(
                batch_size, 1, 1).cuda()  # Values to ignore when gamma i=k
            ...
            E[:, :, 1:self.dim_position, 1:self.dim_position] = (- I * e_last).view(batch_size, n_obs,
                                                                                    self.dim_position - 1,
                                                                                    self.dim_position - 1)
