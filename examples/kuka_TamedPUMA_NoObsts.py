...
def draw_world_axes(origin=(0.0, 0.0, 0.0), axis_len=0.2, line_width=3, life_time=0.0):
    """Draw RGB XYZ axes in the current PyBullet debug visualizer."""
    ox, oy, oz = origin
    pybullet.addUserDebugLine([ox, oy, oz], [ox + axis_len, oy, oz], [1, 0, 0], lineWidth=line_width, lifeTime=life_time)  # X (red)
    pybullet.addUserDebugLine([ox, oy, oz], [ox, oy + axis_len, oz], [0, 1, 0], lineWidth=line_width, lifeTime=life_time)  # Y (green)
    pybullet.addUserDebugLine([ox, oy, oz], [ox, oy, oz + axis_len], [0, 0, 1], lineWidth=line_width, lifeTime=life_time)  # Z (blue)
    pybullet.addUserDebugText("X", [ox + axis_len, oy, oz], [1, 0.2, 0.2], textSize=1.2, lifeTime=life_time)
    pybullet.addUserDebugText("Y", [ox, oy + axis_len, oz], [0.2, 1, 0.2], textSize=1.2, lifeTime=life_time)
...
class example_kuka_TamedPUMA(ExampleGeneric):
    ...
    def run_kuka_example(self):
        ...
        # Draw world-frame axes once (RGB = XYZ) for orientation.
        # If your scene origin isn't useful, change `origin` to your robot base pose.
        draw_world_axes(origin=(0.0, 0.0, 0.0), axis_len=0.25, line_width=4, life_time=0.0)

        # Draw another set at 1.0, 0.0, 0.0
        draw_world_axes(origin=(1.0, 0.0, 0.0), axis_len=0.25, line_width=4, life_time=0.0)
...
def main(render=True, record_video=False, video_path="simulation_video.mp4"):
    ...
    # positions_obstacles_list = [      # @User
    #     [[0.5, 0.15, 0.05], [0.5, 0.15, 0.2]],
    # ]
    # speed_obstacles_list = [          # @User
    #     [[0., 0., 0.], [0., 0., 0.]],
    # ]  
    positions_obstacles_list = [[]]  # @uncomment
    speed_obstacles_list = [[]]  # @uncomment
    goal_pos_list = [
        [0.58, -0.214, 0.115],
    ]
    goal_vel_list = [
